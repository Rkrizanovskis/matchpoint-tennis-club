...
window.cleanupGhostPlayer = cleanupGhostPlayer;
window.viewTemplate = viewTemplate;
window.cleanupTemplate = cleanupTemplate;

// TEMP: Cleanup function for ghost player
async function cleanupGhostPlayer() {
    const playerId = prompt('Enter player ID to remove from Tuesday 20:00-21:00:', 'kristines_1768246441500');
    if (!playerId) return;
    
    if (!confirm(`Remove "${playerId}" from Tuesday 20:00-21:00 slot?`)) return;
    
    try {
        await removePlayer('tuesday', '20:00-21:00', playerId);
        alert('✅ Player removed! Refresh the page.');
    } catch (e) {
        alert('Error: ' + e.message);
    }
}

// View season template
async function viewTemplate() {
    ...
    console.log('Season Template:', template);
    alert('Season Template (check console for full details):\n\n' + 
        'Tuesday 20:00: ' + JSON.stringify(template?.tuesday?.['20:00-21:00']?.players || []) + '\n' +
        'Tuesday 21:00: ' + JSON.stringify(template?.tuesday?.['21:00-22:00']?.players || []) + '\n' +
        'Thursday 20:00: ' + JSON.stringify(template?.thursday?.['20:00-21:00']?.players || []) + '\n' +
        'Thursday 21:00: ' + JSON.stringify(template?.thursday?.['21:00-22:00']?.players || []));
}

// Cleanup template - remove a player ID from all slots
async function cleanupTemplate() {
    const playerId = 'kristines_1768246441500'; // Hardcoded for now
    ...
    try {
        console.log('🧹 Starting cleanup for:', playerId);
        
        // 1. Clean up season template
        ...
        if (templateDoc.exists) {
            ...
            ['tuesday', 'thursday'].forEach(day => {
                ['20:00-21:00', '21:00-22:00'].forEach(time => {
                    if (template[day] && template[day][time] && template[day][time].players) {
                        const idx = template[day][time].players.indexOf(playerId);
                        if (idx > -1) {
                            template[day][time].players.splice(idx, 1);
                            removedFromTemplate = true;
                            console.log(`✅ Removed from template ${day} ${time}`);
                        }
                    }
                });
            });
            ...
            if (removedFromTemplate) {
                await database.collection('seasonTemplate').doc('default').set(template);
                console.log('✅ Template saved!');
            }
        }
        
        // 2. Clean up ALL schedule documents that contain this player
        console.log('🔍 Searching schedules for ghost player...');
        const schedulesSnapshot = await database.collection('schedules').get();
        
        for (const doc of schedulesSnapshot.docs) {
            const data = doc.data();
            if (data.players && data.players.includes(playerId)) {
                const newPlayers = data.players.filter(p => p !== playerId);
                await database.collection('schedules').doc(doc.id).update({ players: newPlayers });
                console.log(`✅ Removed from schedule: ${doc.id}`);
            }
        }
    ...
    if (firebaseEnabled && database) {
        ...
        try {
            await database.runTransaction(async (transaction) => {
                ...
                console.log('  Current Players in slot:', currentPlayers);
                console.log('  Session ID:', sessionId);
                console.log('  Includes check result:', currentPlayers.includes(selectedPlayerId));
        ...
        <div class="time-slot ${statusClass}" ${isAvailable && !isPast ? `onclick="openBookingModal('${day}', '${time}')"` : ''}>
            ...
            <div class="players-list">
                ${slotData.players.map(playerId => {
                    ...
                    if (player) { return `<span class="player-tag ${player.skillLevel}" ${clickAction}>${player.name}${removeX}</span>`; } else { return `<span class="player-tag unknown" ${clickAction}>⚠️ Unknown: ${playerId.substring(0,15)}...${removeX}</span>`; }
                }).join('')}
            </div>
